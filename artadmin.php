  ...
  <body>
    ...
    <!--Header Top-->
     ...
     <nav class="navbar bg-success navbar-fixed-top">
        ...
        <a class="nav-link btn btn-outline-light font-weight-bold" href="insert_art.php">Insert Art</a>
     </nav>
    <!--Header Top--->
    <!--Header Buttom-->
                  ...
                  <div class="mr-auto" style="position: relative; margin-top: 30px; margin-left: 10px;">
                      <div class="card Small shadow" style="width: 200px; background-color: #0A564C">
                        <div class="artadminimg">
                         <a href="dataart.php?id_artt=<?= $que["id"]; ?>"><img src="gambar_art/<?= $que["gambar"]; ?>" class="card-img-top" alt="<?= $que["nama"]; ?>" style="width:200px; height:200px; "></a> 
                        </div>
                        <div class="card-body text-center">
                            <a class="card-title text-light" href="dataart.php?id_artt=<?= $que["id"];?>" style="font-family: Times New Roman; font-size: 18px;"><?php echo $que["nama"]; ?></a><br>
                          <?php foreach($option_jenis_art as $qui) : ?>
                             <?php if($qui['id_jenis_art'] == $que['id_jenis_art']) : ?>
                              <a class="text-light" href="dataart.php?id_artt=<?= $que["id"];?>" style="font-family: Times New Roman; font-size: 18px; text-transform: capitalize;"><?= $qui['jenis_art']; ?></a>
                             <?php endif;?>
                          <?php endforeach;?>
                          <p class="card-text"></p>
                          <a href="update_art.php?id_upart=<?= $que["id"]; ?>" class="btn btn btn-secondary btn-sm" style="font-family: Times New Roman; font-size: 12px">Update</a>
                          <a href="php.php?id_art=<?= $que["id"]; ?>" class="btn btn-danger btn-sm" style="font-family: Times New Roman; font-size: 12px">Delete</a>
                        </div>
                      </div>
                  </div>
                 <?php $a++; 
