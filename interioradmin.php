  ...
  <body>
    ...
    <!--Header Top-->
      ...
      <nav class="navbar bg-success navbar-fixed-top">
        ...
        <a class="nav-link btn btn-outline-light font-weight-bold" href="insert_interior.php">Insert Interior</a>
      </nav>
    <!--Header Top-->
    <!--Header Buttom-->
                  ...
                  <div class="mr-auto" style="position: relative; margin-top: 30px; margin-left: 10px;">
                      <div class="card Small shadow" style="width: 200px; background-color: #0A564C">
                        <div class="artadminimg">
                         <a href="datainterior.php?id_interiorr=<?= $que["id"];?>"><img src="gambar_interior/<?= $que["gambar"]; ?>" class="card-img-top" alt="<?= $que["nama"]; ?>" style="width:200px; height:200px; "></a> 
                        </div>
                        <div class="card-body text-center">
                            <a class="card-title text-light" href="datainterior.php?id_interiorr=<?= $que["id"];?>" style="font-family: Times New Roman; font-size: 18px;"><?php echo $que["nama"]; ?></a><br>
                          <?php foreach($option_jenis_interior as $qui) : ?>
                             <?php if($qui['id_jenis_interior'] == $que['id_jenis_interior']) : ?>
                              <a class="text-light" href="datainterior.php?id_interiorr=<?= $que["id"];?>" style="font-family: Times New Roman; font-size: 18px; text-transform: capitalize;"><?= $qui['jenis_interior']; ?></a>
                             <?php endif;?>
                          <?php endforeach;?>
                          <p class="card-text"></p>
                          <a href="update_interior.php?id_upinterior=<?= $que["id"]; ?>" class="btn btn btn-secondary btn-sm" style="font-family: Times New Roman; font-size: 12px">Update</a>
                          <a href="php.php?id_interior=<?= $que["id"]; ?>" class="btn btn-danger btn-sm" style="font-family: Times New Roman; font-size: 12px">Delete</a>
                        </div>
                      </div>
                  </div>
                 <?php //$a++; 
